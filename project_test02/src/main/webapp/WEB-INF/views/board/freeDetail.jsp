		...
		<div class="boardDetail-content">
			...
			<ul class="boardDetail-content-item boardDetail-content-title">
				<li class="boardcontent-title-item boardcontent-title">
					...
					${freeboard.freetitle}</li>
				<li>
					<div class="freeDetail-btn-div">
						<sec:authorize access="hasRole('ROLE_USER')">
							<c:if test="${freeboard.users.num==principal.user.num}">
								<a href="/board/${freeboard.freenum}/freeUpdateForm"
									class="freeboardbtn freeboardbtn-update"><i
									class="fa-solid fa-pencil"></i></a>
								<button id="freeboardbtn-delete"
									class="freeboardbtn freeboardbtn-delete">
									<i class="fa-solid fa-trash"></i>
								</button>
							</c:if>
							<c:if test="${freeboard.users.num!=principal.user.num}">
									<div id="recommendtest">
								<c:if test="${recommendexist==false}">
										<form>
											<input type="hidden" value="${freeboard.freenum}"
												id="recommendfreenum" size="0"> <input type="hidden"
												value="${principal.user.userid}" id="recommenduserid"
												size="0">
										</form>
								...
								<div id="recommendtest2">
									<c:if test="${recommendexist==true}">
										<button id="freeboardbtn-recommendDelete"
											class="freeboardbtn freeboardbtn-recommend" type="button"
											onclick="recommendDelete(${freeboard.freenum})">
											<i class="fa-solid fa-heart"></i>
										</button>
										<input type="hidden" value="" id="recommendfreenum">
										<input type="hidden" value="" id="recommendid">
									</c:if>
								</div>
			...
			<ul class="boardDetail-content-item boardDetail-info">
				<li class="boardcontent-title-item boardcontent-num info_group">
					<span class="info_detail">NUM : </span><span id="freenum"
					class="info_detail_content"> ${freeboard.freenum} </span>
				</li>
				<li class="boardcontent-title-item boardcontent-username info_group">
					<span class="info_detail">WRITER : </span><span
					class="info_detail_content"> ${freeboard.users.username} </span>
				</li>
				<li class="boardcontent-title-item boardcontent-username info_group">
					<span class="info_detail">작성일 : </span><span
					class="info_detail_content"> <fmt:formatDate value="${freeboard.freecreateDate}" pattern="yyyy-MM-dd"/> </span>
				</li>
				<li class="boardcontent-title-item boardcontent-count info_group">
					<span class="info_detail">VIEW : </span><span id="count"
					class="info_detail_content"> ${freeboard.count} </span>
				</li>
				<li class="boardcontent-title-item boardcontent-count info_group">
					<span class="info_detail">LIKE : </span><span
					class="info_detail_content"> ${freeboard.recommendcnt} </span>
				</li>
			</ul>
			<div class="boardDetail-content-item boardDetail-content-main">
				${freeboard.freecontent}</div>
		...
		<div class="freereply-content">
			...
			<div class="freereply-form-div">
				...
				<form class="freereply-form">
					...
					<div class="freereply-username">
						작성자: <input type="text" id="replyuser"
							value="${principal.user.username}" disabled />
					</div>
					<div class="freereply-comment dummy">
						<textarea id="freereplycontent" class="reply-form-note" rows="5"
							cols="130" placeholder="댓글을 입력하세요."></textarea>
						<button id="freeboardreplybtn-save" class="freeboard-reply-btn"
			...
			<div class="freereply-contents-div">
				<div class="freereply-contents-header">
					<a href="javascript:doDisplay()">댓글 목록
						(${freeboard.freereplycnt}) <i id="chevron-down"
						class="fa-sharp fa-solid fa-chevron-down"></i> <i id="chevron-up"
						class="fa-sharp fa-solid fa-chevron-up"></i>
					</a>
					<div id="myDIV">
						<!-- c:import -->
						<c:import url="/auth/freeboard/${freeboard.freenum}/freereply">
							<%-- http://localhost:8013/auth/freereplyboard/${freeboard.freenum}/freereply --%>
