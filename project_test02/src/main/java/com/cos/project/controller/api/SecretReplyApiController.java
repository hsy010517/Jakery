...
public class SecretReplyApiController {
 ...
 SecretReplyService replyService;
	...
	/*
	 * @PostMapping("/api/freeboard/{freereplynum}/freereply") public
	 * ResponseDto<Integer> replySave(@PathVariable int freereplynum, @RequestBody
	 * FreeReplys freereply,
	 * 
	 ...
	 * 
	 * replyService.일대일게시판댓글쓰기(freereplynum, freereply, principal.getUser()); return
	 * new ResponseDto<Integer>(HttpStatus.OK.value(),1); }
	 * 
	 * //댓글 삭제
	 * 
	 * @DeleteMapping("/api/freeboard/freereply/{deletenum}") public
	 * ResponseDto<Integer> replyDelete(@PathVariable int deletenum){
	 * System.out.println("댓글 삭제1"+deletenum); replyService.일대일게시판댓글삭제(deletenum);
